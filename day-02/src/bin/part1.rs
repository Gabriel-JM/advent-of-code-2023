...
impl<'a> Game<'a> {
    fn validate_cube_set(&self, map: &BTreeMap<&'a str, u32>) -> Option<u32> {
        self.rounds
            .iter()
            .all(|round| {
                round.iter().all(|shown_cube| {
                    shown_cube.amount <= *map.get(shown_cube.color).expect("a valid cube color")
                })
            })
            .then_some(self.id.parse::<u32>().expect("Game id to be a number"))
    }
}
...
fn parse_games(input: &str) -> Result<(&str, Vec<Game>), Error> {
    let games = input.lines().map(|line| {
        ...
        let game_id = game_id_and_list[0];
...
fn process(input: &str) -> String {
    ...
    games
        .1
        .iter()
        .filter_map(|game| game.validate_cube_set(&map))
        .sum::<u32>()
        .to_string()    
}
